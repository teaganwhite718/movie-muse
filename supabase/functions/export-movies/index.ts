...
serve(async (req) => {
  ...
  try {
    ...
    while (true) {
      const { data, error } = await supabase
        .from("movie_chunks")
        .select("movie_title, release_year, genre, director, section, text, chunk_index, document_id, embedding")
        .order("movie_title")
        .range(page * pageSize, (page + 1) * pageSize - 1);
      if (error) throw error;
      if (!data || data.length === 0) break;
    ...
    const embeddedCount = allChunks.filter(c => c.embedding !== null).length;
    ...
    return new Response(
      JSON.stringify({
        metadata: {
          ...
          chunks_with_embeddings: embeddedCount,
